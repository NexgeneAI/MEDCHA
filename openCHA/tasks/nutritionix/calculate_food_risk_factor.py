...
class CalculateFoodRiskFactor(BaseTask):
    ...
    **Description:**
    ...
    description: str = (
        "Calculates the daily foods risk factor for diabetes using evidence based nutritional method."
    )
    ...
    def check_rules(self, total_intake):
        ...
        carb = (
            total_intake["Total_Carbohydrate"] * carbohydrates_calorie_ratio
        ) / total_intake["Calories"]
        fat = (total_intake["Total_Fat"] * fat_calorie_ratio) / total_intake["Calories"]
        saturated_fat = (
            total_intake["Saturated_Fat"] * saturated_fat_calorie_ratio
        ) / total_intake["Calories"]
        protein = (total_intake["Protein"] * protein_calorie_ratio) / total_intake[
            "Calories"
        ]
        ...
        is_fiber_risk = (
            total_intake["Dietary_Fiber"] < 20 or total_intake["Dietary_Fiber"] > 35
        )

        return {
            "Total_Carbohydrate_Risk": (
                "No Risk"
                if not is_carb_risk
    ...
    def process_nutrients(self, nutrition_json):
        ...
        for food in nutrition_json["foods"]:
            total_nutrients["Calories"] += (
                food["nf_calories"]
                if ("nf_calories" in food) and (food["nf_calories"] is not None)
                else 0
            )
            total_nutrients["Total_Fat"] += (
                food["nf_total_fat"]
                if ("nf_total_fat" in food) and (food["nf_total_fat"] is not None)
                else 0
            )
            ...
            total_nutrients["Cholesterol"] += (
                food["nf_cholesterol"]
                if ("nf_cholesterol" in food) and (food["nf_cholesterol"] is not None)
                else 0
            )
            total_nutrients["Sodium"] += (
                food["nf_sodium"]
                if ("nf_sodium" in food) and (food["nf_sodium"] is not None)
                else 0
            )
            ...
            total_nutrients["Sugars"] += (
                food["nf_sugars"]
                if ("nf_sugars" in food) and (food["nf_sugars"] is not None)
                else 0
            )
            total_nutrients["Protein"] += (
                food["nf_protein"]
                if ("nf_protein" in food) and (food["nf_protein"] is not None)
                else 0
            )
            total_nutrients["Potassium"] += (
                food["nf_potassium"]
                if ("nf_potassium" in food) and (food["nf_potassium"] is not None)
                else 0
            )
            total_nutrients["Phosphorus"] += (
                food["nf_p"] if ("nf_p" in food) and (food["nf_p"] is not None) else 0
            )

        risks = self.check_rules(total_nutrients)

        total_nutrients["Total_Fat"] = f'{total_nutrients["Total_Fat"]}g'
        total_nutrients["Saturated_Fat"] = f'{total_nutrients["Saturated_Fat"]}g'
        total_nutrients["Cholesterol"] = f'{total_nutrients["Cholesterol"]}mg'
        total_nutrients["Sodium"] = f'{total_nutrients["Sodium"]}mg'
        total_nutrients["Total_Carbohydrate"] = (
            f'{total_nutrients["Total_Carbohydrate"]}g'
        )
        total_nutrients["Dietary_Fiber"] = f'{total_nutrients["Dietary_Fiber"]}g'
        total_nutrients["Sugars"] = f'{total_nutrients["Sugars"]}g'
        total_nutrients["Protein"] = f'{total_nutrients["Protein"]}g'
        total_nutrients["Potassium"] = f'{total_nutrients["Potassium"]}mg'
        total_nutrients["Phosphorus"] = f'{total_nutrients["Phosphorus"]}mg'
